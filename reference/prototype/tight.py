...
for r,i,j in pairs:
    ...
    if gi is None and gj is None:
        ...
        g=groups[gi]
        if all(rmse(F[j],F[k])<=0.45 for k in g): g.add(j); assigned[j]=gi
    elif gj is not None and gi is None:
        g=groups[gj]
        if all(rmse(F[i],F[k])<=0.45 for k in g): g.add(i); assigned[i]=gj
    elif gi!=gj:
        a,b=groups[gi],groups[gj]
        if all(rmse(F[x],F[y])<=0.45 for x in a for y in b):
            a|=b
...
print(f"max diameter across all clusters: {md:.2f}  (threshold 0.45)")

...
//!
//!     cargo run --release -p openfoto-core --example passes -- <source-library> [n]
//!
...
fn build(src: &PathBuf, n: usize, name: &str) -> Result<PathBuf> {
    ...
    let dir = std::env::temp_dir().join(format!("openfoto-passes-{}-{name}", std::process::id()));
...
fn main() -> Result<()> {
    ...
    openfoto_core::scan::scan(&mut lib, false)?;
    let t = Instant::now();
    openfoto_core::thumbs::build(&lib)?;
    ...
    let t = Instant::now();
    semantic::analyze(&lib, &openfoto_core::progress::silent)?;
    ...
    openfoto_core::scan::scan(&mut lib2, false)?;
    let t = Instant::now();

...
fn main() -> Result<()> {
    ...
    let tmp = std::env::temp_dir().join("openfoto-thumbcheck");
    ...
    for p in photos.iter().step_by(step).take(n) {
        let o = openfoto_core::imageio::orientation(p);
        if o != 1 { oriented += 1; }
        if std::fs::read(p).ok()
            .and_then(|b| openfoto_core::imageio::embedded_preview(&b, 512)).is_some() { preview += 1; }

        // Reference: decode everything, rotate, then shrink.
        let full = openfoto_core::imageio::load_rgb(p)?;

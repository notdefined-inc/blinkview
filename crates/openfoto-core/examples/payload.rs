...
fn main() -> Result<()> {
    ...
    let mut lib = openfoto_core::Library::open(&root)?;

    let t = Instant::now();
    let rows = lib.index.all()?;
    let rows_ms = t.elapsed().as_secs_f64() * 1000.0;

    // First touch walks every folder for an openfoto.json; after that it is held.
    let t = Instant::now();
    let _ = lib.user_data()?;
    ...
    let out: Vec<PhotoInfo> = rows
        .iter()
        .map(|r| {
            let folder = openfoto_core::plan::folder_of(&r.path).to_string();
            ...
            PhotoInfo {
                ...
                name,
                folder,
                thumb: openfoto_core::thumbs::thumb_path_at(&root, &r.hash).display().to_string(),
                taken_at: r.taken_at,
                faces: 0,
                people: vec![],
                width: 0,

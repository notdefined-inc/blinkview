...
pub fn write_gps(path: &Path, lat: f64, lon: f64) -> Result<()> {
    ...
    if !matches!(ext.as_str(), "jpg" | "jpeg") {
        bail!(
            "openfoto can only write a location into JPEG files, not {}",
    ...
    // exactly 0,0 as no fix, so writing it would produce a file openfoto itself reads
    // as having no location. Refusing to write what we cannot read keeps the two
    // halves of this module honest with each other.
    if lat == 0.0 && lon == 0.0 {
        bail!("0, 0 is what a camera writes when it has no fix, so openfoto reads it as no location");
    }
    ...
    let tmp = path.with_extension("openfoto-gps-tmp");
...
mod tests {
    ...
    fn null_island_is_refused_because_it_could_not_be_read_back() {
        ...
        // openfoto itself reads as unlocated. The two halves must agree.
    ...
    fn a_rewrite_that_cannot_be_read_back_leaves_the_original_alone() {
        ...
        assert!(!d.join("broken.openfoto-gps-tmp").exists());
        let _ = std::fs::remove_dir_all(&d);
    }

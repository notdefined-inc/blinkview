...
fn fixture(name: &str) -> Option<PathBuf> {
    ...
    let dir = std::env::temp_dir().join(format!("blinkview-ap-{}-{name}", std::process::id()));
...
fn models_ready() -> bool {
    blinkview_core::faces::models::find("yunet.onnx").is_ok()
}
...
/// A file blinkview cannot read must be given up on *once*, not on every pass.
///

...
//!
//! This lives at the **library root**, not inside `.blinkview/`, and that placement is
...
//! calls disposable would mean `rm -rf .blinkview` silently destroys work.
//!
...
pub const FILE: &str = "blinkview.json";

/// What the same file was called before the rename (ADR-0017).
pub const LEGACY_FILE: &str = "openfoto.json";

/// The metadata file in `dir`, adopting a pre-rename one when that is all there is.
///
/// Renamed as it is read rather than left in place: the cascade would otherwise have
/// to consult two names in every folder for the life of the program. A folder holding
/// both is already current, and the old file is left alone rather than merged.
pub(crate) fn file_in(dir: &Path) -> std::path::PathBuf {
    ...
    match std::fs::rename(&legacy, &current) {
        Ok(()) => current,
        // A read-only volume can still be read under the old name.
        Err(_) => legacy,
    }
}
...
pub struct FolderView {
    ...
    /// the reader rather than rejected, so a newer blinkview writing a sort this one
...
/// The cascade: one `blinkview.json` per folder, nearest wins (ADR-0010).
///
...
impl UserDataSet {
    /// Read every `blinkview.json` at or below `root`.
    ///
...
mod tests {
    ...
    fn a_copied_folder_carries_its_metadata() {
        ...
        std::fs::copy(
            src.path().join("Trip/Greece Day3/blinkview.json"),
            dst.path().join("blinkview.json"),
        )
        .unwrap();

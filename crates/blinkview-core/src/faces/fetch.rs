...
/// copy and deleting a library's `.blinkview` never forces a re-download.
pub fn cache_dir() -> Result<PathBuf> {
    let home = std::env::var_os("HOME").context("HOME is not set")?;
    Ok(PathBuf::from(home).join(".cache/blinkview/models"))
}
...
mod tests {
    ...
    fn cache_dir_is_under_the_user_cache() {
        ...
        assert!(d.ends_with("blinkview/models"), "{}", d.display());
    }
}

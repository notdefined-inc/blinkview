...
fn main() -> Result<()> {
    ...
    for p in photos.iter().step_by(step).take(n) {
        ...
        if std::fs::read(p)
            .ok()
            .and_then(|b| blinkview_core::imageio::embedded_preview(&b, 512))
            .is_some()
        {
            preview += 1;
        }
        ...
        let d: f64 = got
            ...
            .sum::<f64>()
            / (got.width() * got.height() * 3) as f64;
        if d > 24.0 {
            diff += 1;
            worst.push((p.file_name().unwrap().to_string_lossy().into(), d));
        } else {
            same += 1;
        }
    }
    println!(
        "checked {}  |  embedded preview used: {preview}  |  EXIF-rotated: {oriented}",
        same + diff
    );

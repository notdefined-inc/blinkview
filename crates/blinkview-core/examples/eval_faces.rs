...
use blinkview_core::faces::{assign, people::People, pipeline};
use blinkview_core::{scan, Library};
...
fn main() -> anyhow::Result<()> {
    let root = std::env::args()
        .nth(1)
        .expect("usage: eval_faces <library> [seeds]");
    ...
    let hash_to_folder: BTreeMap<String, String> = lib
        .index
        .all()?
        .into_iter()
        .map(|r| (r.hash, folder(&r.path)))
        .collect();
    ...
    println!(
        "\nseeded {} people with {seeds} references each",
        people.people.len()
    );
    ...
    println!(
        "  correct   {right:>4}  ({:.0}%)",
        100.0 * right as f32 / total as f32
    );
    println!(
        "  WRONG     {wrong:>4}  ({:.0}%)",

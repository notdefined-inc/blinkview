...
fn main() -> Result<()> {
    ...
    let t = Instant::now();
    for p in &sample {
        let _ = enc.embed_image(p);
    }
    let seq = t.elapsed().as_secs_f64();
    println!(
        "sequential   {:>6.1} ms/photo   {:>5.1} photos/s",
        seq * 1000.0 / sample.len() as f64,
    ...
    pool.install(|| {
        sample
            .par_chunks(sample.len().div_ceil(cores))
            .for_each(|chunk| {
                if let Ok(mut e) = blinkview_core::semantic::Encoder::load() {
                    for p in chunk {
                        let _ = e.embed_image(p);
                    }
                }
            });
    });
    ...
    println!(
        "parallel     {:>6.1} ms/photo   {:>5.1} photos/s   ({:.1}x)",
        par * 1000.0 / sample.len() as f64,
        sample.len() as f64 / par,
        seq / par
    );
    ...
    for (label, secs) in [("sequential", seq), ("parallel", par)] {
        ...
        println!(
            "  {label:<11} 80,000 assets -> {:>5.0} min      200,000 -> {:>5.1} h",
            per * 80_000.0 / 60.0,
            per * 200_000.0 / 3600.0
        );
    }
    Ok(())
}

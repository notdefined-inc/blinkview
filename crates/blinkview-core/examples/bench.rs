...
//!
//!     cargo run --release -p blinkview-core --example bench -- <library> [sample]
...
fn main() -> Result<()> {
    ...
    let tmp = std::env::temp_dir().join(format!("blinkview-bench-{}", std::process::id()));
    ...
    let preview = timed("embedded preview (no full decode)", &sample, |p| {
        let b = std::fs::read(p)?;
        blinkview_core::imageio::embedded_preview(&b, 512)
    ...
    println!("\n--- face detection ---");
    if let Ok(model) = blinkview_core::faces::models::find("yunet.onnx") {
        let mut det = blinkview_core::faces::detect::Detector::load(&model)?;
    ...
    println!("\n--- semantic embedding ---");
    if blinkview_core::semantic::Encoder::available() {

...
import { hydrate, isRaw, needsConversion, inFolder, DAY, TIME, computeLayout as coreComputeLayout, sectionFor as coreSectionFor, loadGuard, MONTHS, LABEL_NAMES, parseQuery, matchesStructured, bytesLabel, cosine, labelColour, monthKey, project, unproject, straightenZoom, INTO, stripFiller, verbOf, nearestVerb, splitClauses, parseClause, isCommand } from "./core.js";
...
/* Load-race guards over this UI's source state (core.js loadGuard). */
const { begin: beginLoad, stillCurrent } = loadGuard(() => S.source);
const TRASH = "Trash";
...
function sectionFor(p) { return coreSectionFor(p, S.folder || ""); }

function computeLayout(width) {
  LAYOUT = coreComputeLayout(S.view,
    { sort: S.sort, peek: !!S.peek, group: S.group, folder: S.folder || "", rowH: ROW_H, gap: GAP, headH: HEAD_H },
    width);
}
